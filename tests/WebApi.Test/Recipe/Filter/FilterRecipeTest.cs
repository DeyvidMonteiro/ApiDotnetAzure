...
public class FilterRecipeTest : MyRecipeBookClassFixture
{
    ...
    private MyRecipeBook.Domain.Enums.Difficulty _recipeDifficultLevel;
    ...
    public FilterRecipeTest(CustomWebApplicationFactory factory) : base(factory)
    {
        ...
        _recipeDifficultLevel = factory.GetRecipeDifficulty();
    ...
    public async Task Success()
    {
        var request = new RequestFilterRecipeJson
        {
            ...
            Difficulties = [(MyRecipeBook.Communication.Enums.Difficulty)_recipeDifficultLevel],
    ...
    public async Task Success_NoContent()
    {
        ...
        var taken = JwtTokenGeneratorBuilder.Build().Generate(_userIdentifier);

        var response = await DoPosts(method: METHOD, request: request, token: taken);
    ...
    public async Task Error_CookingTime_Invalid(string culture)
    {
        ...
        var response = await DoPosts(METHOD, request: request, token: token, culture: culture);
