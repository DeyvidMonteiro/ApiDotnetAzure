...
public class GetDashboardUseCase : IGetDashboardUseCase
{
    ...
    public GetDashboardUseCase(IRecipeReadOnlyRepository repository,
        IMapper mapper,
    ...
    {
        var logggedUser = await _loggedUser.User();

        var recipes = await _repository.GetForDashboard(logggedUser);
