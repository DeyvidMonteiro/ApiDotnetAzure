...
public class RecipeValidator : AbstractValidator<RequestRecipeJson>
{
    public RecipeValidator()
    {
        ...
        RuleFor(recipe => recipe.Ingredientes.Count).GreaterThan(0).WithMessage(ResourceMessagesExceptions.AT_LAST_ONE_INGREDIENT);
        RuleFor(recipe => recipe.Instructions.Count).GreaterThan(0).WithMessage(ResourceMessagesExceptions.AT_LAST_ONE_INSTRUCTION);
        RuleForEach(recipe => recipe.DishTypes).IsInEnum().WithMessage(ResourceMessagesExceptions.DISH_TYPE_NOT_SUPPORTED);
        RuleForEach(recipe => recipe.Ingredientes).NotEmpty().WithMessage(ResourceMessagesExceptions.INGREDIENT_EMPTY);
        ...
        RuleFor(recipe => recipe.Instructions)
            ...
            .WithMessage(ResourceMessagesExceptions.TWO_OR_MORE_INSTRUCTIONS_SAME_ORDERS);
    }
}
